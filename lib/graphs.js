...
module.exports = ( function() {
	...
	Private.database.relationship.update = function( req ) {
		...
		var id = req.request.id;
		var generic_callback = function( err, result ) {
			console.log( 'Private.database.relationship.update > generic_callback', err, result );
			if( 'undefined' !== typeof err && null !== err ) {
				own_on_error( err );
			} else {
				console.log( 'generic callback success > ' + JSON.stringify( { id: result.id, data: result.data } ) );
	...
	Private.database.index.update = function( req ) {
		...
		var generic_callback = function( err, result ) {
			...
			if( 'undefined' !== typeof err && null !== err ) {
				...
				//console.log('Private.database.index.update',result);

				if ( result.length > 0 ) {	
					target = result.length;
					for ( var x = 0; x < result.length; x += 1 ) {
						if ( 'relationship' === type ) {
							console.log( "TYPE IS REL DOING UPDATE", result[ x ].id );
							...
							console.log( "TYPE IS NODe DOING UPDATE", result[ x ].id );
