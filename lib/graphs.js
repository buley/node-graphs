					...
					if ( true === reindex ) {
						...
						if ( 'undefined' !== typeof res2.reference.index ) {
							...
							var generic_callback_2 = function( err2, result2 ) {
								...
								console.trace();
								if( 1===2&&'undefined' !== typeof err2 && null !== err2 ) {
									own_on_error( err2 );
								} else {
									...
									console.log("WTFFFFFFFFFFFFFFFF",obj);
									//own_on_success( obj );

								}
							};
							console.log("ENTERING"+ JSON.stringify( data ) );
							for ( var attr in data ) {
								if ( data.hasOwnProperty( attr ) ) {
							...
							console.log("CANNED INDEX CREATE");
				...
				if ( result.length > 0 ) {	
					...
					for ( var x = 0; x < result.length; x += 1 ) {
						if ( 'relationship' === type ) {
							console.log('incoming Private.database.relationship.update RELATIONSHIP' );
