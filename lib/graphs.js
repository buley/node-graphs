			...
			if( true === req.reindex ) {	
				...
				for( x = 0; x < xlen; x += 1 ) {
					...
					var very_own_on_success = function( err3, res3 ) {
						...
						Private.log( 'Private.relationship.update > own_on_complete() > reindex > very_own_on_success() > WALDO', err3, res3 );
						if ( 'function' === typeof req.on_complete ) {
							req.on_complete( req, res );
						}				
					};
					...
					req.index = index;
					for ( var attr3 in res.data ) {
						req.key = attr3;
						req.value = res.data[ attr3 ];
						Private.log( 'Private.relationship.update > own_on_complete() > reindex > doing Private.database.index.update()', req );
						Private.database.index.update( { request: req, on_success: very_own_on_success, on_error: own_on_error } );
					}
				}
			} else {
