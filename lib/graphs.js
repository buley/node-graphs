...
module.exports = ( function() {
	...
	Private.database.index.update = function( req ) {
		...
		if ( false !== reindex || 'undefined' === typeof reference ) {
			if ( 'relationship' === type ) {
	...
	Private.node.update = function( req ) {
		...
		var own_on_complete = function( res, count ) {
			...
			if( true === req.reindex ) {	
				...
				req.reindex = false;
				Private.database.index.update( { request: req, on_success: very_own_on_success, on_error: own_on_error, reference: req.reference } );
			} else {
				if ( 'function' === typeof req.on_complete ) {
					req.on_complete( req, results, count );
				}
