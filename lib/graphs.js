...
module.exports = ( function() {
	...
	Private.database.index.create = function( req ) {
		...
		console.log('Private.database.index.create',req);
		var data = req.request.data;
		var id = req.request.id;
		...
		if ( type === 'relationship' ) {
			console.log('Private.db.getRelationshipById()',id);
			Private.db.getRelationshipById( id, generic_callback );
		} else {
			console.log('Private.db.getNodeById()',id);
			Private.db.getNodeById( id, generic_callback );
		}
	};
			...
			if( 'undefined' !== typeof err && null !== err ) {
				...
				var very_own_on_success = function( res2 ) {
					...
					if ( true === reindex ) {
						...
						if ( 'undefined' !== typeof res2.reference && null !== res2.reference ) {
							requ.reference = res2.reference;
						}
						console.log("Private.database.index.create > REQUESTING", requ );
						Private.database.index.create( requ );
