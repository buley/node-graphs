	...
	Private.database.index.update = function( req ) {
		...
		var generic_callback = function( err, result ) {
			...
			if( 'undefined' !== typeof err && null !== err ) {
				...
				var own_generic_callback = function( err2, result2 ) {
					console.log( 'Private.database.index.update > own_generic_callback', err2, result2 );
				};

				var very_own_on_success = function( res2 ) {

					console.log( 'Private.database.index.update > very_own_on_success', res2 );
					var id = res2.id; 

					var very_very_own_on_success = function( res3 ) {
						console.log('Private.database.index.update > very_very_own_on_success',res3);
						count = count + 1;
						Private.log( 'Private.database.index.update > very_very_own_on_success', res3 );
						if ( x === target ) {
							own_on_success( res3 );
						}	
					};
					...
					var very_very_own_on_error = function( res3 ) {
						console.log( 'Private.database.index.update > very_very_own_on_error', res3 );
						if ( x === target ) {
							own_on_error( res3 );
						}	
					};
					...
					if ( true === reindex ) {
								...
								console.log( "DOING Private.db.deleteRelationshipEntry()", index,key,value);
								Private.db.deleteRelationshipEntry( index, key, value, function(res3) {

									res.index( index, key, value, function(res2) { console.log("DUMB CALLBACK",res2); 
										Private.database.relationship.update( {
											request: { data: data, id: res.id }
						...
						if ( 'undefined' !== typeof res2.reference.index ) {

							var generic_callback_2 = function( err2, result2 ) {
								console.log( 'Private.database.index.update > generic_callback_2 > err2, result2', err2, result2 );
								if( 'undefined' !== typeof err2 && null !== err2 ) {
									own_on_error( err2 );
								} else {
									...
									if ( null !== result2 ) {
										...
										obj.id = result2.id;
										obj.data = result2.data;

										if ( type === 'relationship' ) {
											obj.start = result2.start.id;
											obj.end = result2.end.id;
										}
							...
							for ( var attr in data ) {
								if ( data.hasOwnProperty( attr ) ) {
									console.log( "DOING INDEX!", index, attr, typeof res2.reference.index );
									res2.reference.index( index, attr, data[ attr ], generic_callback_2 );
								}
							}
						} else {
						...
						console.log('Private.database.index.update > doing very_very_own_on_success()',type);
				...
				console.log('Private.database.index.update > ITERATION',result);
		...
		var own_on_success = function( res ) {
			console.log('Private.database.index.update > own_on_success',type);
			if ( 'function' === typeof req.on_success ) {
				req.on_success( res );
			}
		};
		...
		if ( 'relationship' === type ) {
			console.log('Private.database.index.update > RELATIONSHIP',type);
