...
module.exports = ( function() {
	...
	Private.index.update = function( req ) {
		...
		var idx_callback = function( req1, res1 ) {
			...
			Private.database.index.update( { request: reqobj, on_success: own_on_success, on_error: own_on_error } );
		...
		var very_own_on_complete = function( err3, res3 ) {
			...
			for( x = 0; x < xlen; x += 1 ) {
				...
				Private.log( 'Private.index.update > very_own_on_complete() > item', xitem  );
				for( y = 0; y < xlen; y += 1 ) {
					xitem = res3[ y ];
					/* for each of the nodes found by index, kick off the process */
					Private.log( 'Private.index.update > very_own_on_complete() > item', yitem );
