	...
	Private.database.index.update = function( req ) {
		...
		var generic_callback = function( err, result ) {
			if( 'undefined' !== typeof err && null !== err ) {
				...
				console.log('Private.database.index.update',result2);
				if ( result.length > 0 ) {	
					for ( var x = 0; x < result.length; x += 1 ) {
						console.log("SPILL",result );
						Private.database.node.update( { request: { data: data, id: result[ x ].id }, on_success: very_own_on_success, on_error: very_own_on_error } );
					}
				} else {
					console.log("SPILL",result.id );
					Private.database.node.update( { request: { data: data, id: result.id }, on_success: very_own_on_success, on_error: very_own_on_error } );
				}
