...
module.exports = ( function() {
	...
	Private.index.create = function( req ) {
			...
			if ( Array.isArray( req.data ) ) {
				...
				for( a = 0; a < alen; a += 1 ) {
					Private.database.index.create( { type: req.type, request: req, on_success: own_on_success, on_error: own_on_error } );
				}
	...
	Private.index.destroy = function( req ) {
		...
		if ( Array.isArray( req.data ) ) {
			...
			for( a = 0; a < alen; a += 1 ) {
				Private.database.index.destroy( { type: req.type, request: req, on_success: own_on_success, on_error: own_on_error } );
			}
	...
	Private.index.update = function( req ) {
		...
		if ( Array.isArray( req.data ) ) {
			...
			for( a = 0; a < alen; a += 1 ) {
				Private.database.index.update( { type: req.type, request: req, on_success: own_on_success, on_error: own_on_error } );
			}
	...
	Private.index.read = function( req ) {
		...
		if ( Array.isArray( req.data ) ) {
			...
			target = alen;
			for( a = 0; a < alen; a += 1 ) {
				Private.database.index.read( { type: req.type, request: req, on_success: own_on_success, on_error: own_on_error } );
			}
