...
module.exports = ( function() {
			...
			if ( 'undefined' !== typeof err && null !== err ) {
				...
				var own_generic_callback = function( err2, result2 ) {
					if ( 'undefined' !== typeof err2 && null !== err2 ) {
						console.log( 'Private.database.node.destroy > generic_callback() > error', err2 );
						own_on_error( err2 );
					} else {
	...
	Private.database.index.create = function( req ) {
		...
		console.log( 'Private.database.index.create > id, key, value, type, index', id, key, value, type, index );
	...
	Private.database.index.destroy = function( req ) {
		console.log( 'Private.database.index.destroy', req );
		...
		var generic_callback = function( err, result ) {
			if( 'undefined' !== typeof err && null !== err ) {
				...
				var generic_callback_2 = function( err2, result2 ) {
					if( 'undefined' !== typeof err2 && null !== err2 ) {
						console.log( 'Private.database.index.destroy > error', err2 );
						own_on_error( err2 );
					} else {
						console.log( 'Private.database.index.destroy > success', result2 );
						own_on_success( result2 );
					}
				}
				console.log( 'Private.database.index.destroy > doing result2.delete()', typeof result2.delete );
	...
	Private.database.index.update = function( req ) {
		...
		var generic_callback = function( err, result ) {
			...
			console.log( 'Private.database.index.update > generic_callback() > RESULT', err, result );
			if( 'undefined' !== typeof err && null !== err ) {
				...
				var very_own_on_success = function( res2 ) {
					...
					var very_very_own_on_success = function( res3 ) {
						count = count + 1;
						Private.log( 'Private.database.index.update > very_very_own_on_success', count );
						own_on_success( res3 );
					};
						...
						for ( var attr in data ) {
							if ( data.hasOwnProperty( attr ) ) {
								...
								var vvv_generic_callback = function(err3,res3) {
									console.log('Private.database.index.update > vvv_generic_callback() > doing Private.database.index.create()' );
								...
								console.log( 'Private.database.index.update > doing Private.database.index.destroy()' );	
								Private.database.index.destroy( {
									...
									, on_success: very_very_own_on_error
									, reference: res2.reference
								} );
							}
						}
		...
		if ( 'relationship' === type ) {
			console.log('Private.database.index.update > RELATIONSHIP > Private.db.getIndexedRelationships()', index, key, value );
			Private.db.getIndexedRelationships( index, key, value, generic_callback );
		} else {
			console.log('Private.database.index.update > NODE > Private.db.getIndexedNodes()', index, key, value );
			Private.db.getIndexedNodes( index, key, value, generic_callback );
		}
	...
	Private.index.update = function( req ) {
		...
		var idx_callback = function( req1, res1 ) {
			console.log( 'Private.entities.update > idx_callback() > RESULT', req1, res1  );
			Private.database.index.update( { request: req, on_success: own_on_success, on_error: own_on_error } );
		};

		var lookup_callback = function( req0, res0 ) {
			if ( 'relationship' === req.index_type ) {
				console.log( 'Private.entities.update > doing Private.database.relationship.update()', req0  );
				Private.database.relationship.update( { request: req0, on_success: idx_callback, on_error: own_on_error } );
			} else {
				console.log( 'Private.entities.update > doing Private.database.node.update()', req0  );
				Private.database.node.update( { request: req0, on_success: idx_callback, on_error: own_on_error } );
			}
		};
		...
		if ( Array.isArray( req.data ) && ( 'undefined' === typeof key || 'undefined' === typeof value || null === key || null === value ) ) {
			...
			for( a = 0; a < alen; a += 1 ) {
				Private.index.read( { request: req, on_success: idx_callback, on_error: own_on_error } );
			}
		} else {
			Private.index.read( { request: req, on_success: idx_callback, on_error: own_on_error } );
		}
