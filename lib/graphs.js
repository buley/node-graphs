			...
			if( 'undefined' !== typeof err && null !== err ) {
				...
				if ( result.length > 0 ) {	
					for ( var x = 0; x < result.length; x += 1 ) {
						Private.database.node.update( { request: { data: data, id: result[ x ].id }, on_success: own_on_success, on_error: own_on_error } );
					}
				} else {
					Private.database.node.update( { request: { data: data, id: result.id }, on_success: own_on_success, on_error: own_on_error } );
				}
