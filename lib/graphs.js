...
module.exports = ( function() {
	...
	Private.database.node.destroy = function( req ) {
		...
		Private.log( 'TESTING Private.database.node.destroy', typeof req.on_success, typeof req.on_error );
	...
	Private.database.node.update = function( req ) {
		...
		Private.log( 'TESTING Private.database.node.update', typeof req.on_success, typeof req.on_error );
		...
		var generic_callback = function( err, result ) {
			...
			if ( 'undefined' !== typeof err && null !== err ) {
				...
				var own_generic_callback = function( err2, result2 ) {
					console.log("GANGSTER",err2,result2);
					if ( 'undefined' !== typeof err2 && null !== err2 ) {
						own_on_error( err2 );
					} else {
						own_on_success( result2 );
				...
				result.data = data;
				console.log("GANGSTER COP");
				var res = result.save( own_generic_callback );
			}
		};
	...
	Private.node.destroy = function( req ) {
		...
		if ( Array.isArray( req.data ) ) {
			...
			for( a = 0; a < alen; a += 1 ) {
				console.log("CALLING Private.database.node.destroy > a ", a );
				Private.database.node.destroy( { request: req, on_success: own_on_success, on_error: own_on_error } );
			}

		} else {

				console.log("CALLING Private.database.node.destroy > b" );
			Private.database.node.destroy( { request: req, on_success: own_on_success, on_error: own_on_error } );
	...
	Public.prototype.destroy = function( req ) {
		...
		delete req.datatype;
		if ( 'node' === datatype ) {
			console.log('Calling Private.node.destroy via Public.prototype.destroy',req);
