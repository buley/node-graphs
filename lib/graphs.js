...
module.exports = ( function() {
	...
	Private.index.update = function( req ) {
		...
		if ( Array.isArray( req.data ) && ( 'undefined' === typeof key || 'undefined' === typeof value || null === key || null === value ) ) {
			...
			for( a = 0; a < alen; a += 1 ) {
				if ( 'relationship' === type ) {
					Private.database.relationship.update( { request: req, on_success: idx_callback, on_error: own_on_error } );
				} else {
					Private.database.node.update( { request: req, on_success: idx_callback, on_error: own_on_error } );
				}
			}
		} else {
			if ( 'relationship' === req.index_type ) {
				console.log( 'Private.entities.update > doing Private.database.relationship.update()', req  );
				Private.database.relationship.update( { request: req, on_success: idx_callback, on_error: own_on_error } );
			} else {
				console.log( 'Private.entities.update > doing Private.database.node.update()', req  );
				Private.database.node.update( { request: req, on_success: idx_callback, on_error: own_on_error } );
			}
		}
