...
module.exports = ( function() {
	...
	Private.index.update = function( req ) {
		...
		var lookup_callback = function( req0, res0 ) {
			console.log( 'Private.index.update > Private.database.node.update() > lookup_callback()', res0  );
			if ( 'relationship' === req.index_type ) {
				console.log( 'Private.index.update > doing Private.database.relationship.update()', res0  );
				Private.database.relationship.update( { request: req, on_success: idx_callback, on_error: own_on_error, reference: res0 } );
			} else {
				console.log( 'Private.index.update > doing Private.database.node.update()', res0  );
		...
		if ( Array.isArray( req.data ) && ( 'undefined' === typeof key || 'undefined' === typeof value || null === key || null === value ) ) {
			...
			target = alen;
			for( a = 0; a < alen; a += 1 ) {
				Private.index.read( { request: req.request, on_success: lookup_callback, on_error: own_on_error } );
			}
		} else {
			console.log( 'Private.index.update > doing Private.index.read()', req  );
			Private.index.read( { request: req.request, on_success: lookup_callback, on_error: own_on_error } );
		}
