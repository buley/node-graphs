				...
				if ( result.length > 0 ) {	
					...
					for ( var x = 0; x < result.length; x += 1 ) {
						if ( 'relationship' === type ) {
							...
							Private.log( 'Private.database.index.update > ITERATION > Private.database.relationship.update() > INDEXED', result[ x ].data.indexed );
		...
		if( ( 'undefined' === typeof key || null === key || 'undefined' === typeof value || null === value ) && 'undefined' !== typeof reference && null !== reference ) {
			...
			for ( var attr in data ) {
				if ( data.hasOwnProperty( attr ) ) {
					...
					Private.log( 'Private.database.index.update > doing Private.database.index.destroy()', reference.data );	
					Private.database.index.destroy( {
						request: {
							index: index
							, id: id
