...
module.exports = ( function() {
	...
	Private.database.node.create = function( req ) {
		...
		var generic_callback = function( err, result ) {
			...
			if( 'undefined' !== typeof err && null !== err ) {
				...
				Private.log( 'generic callback success > ' + JSON.stringify( { id: node.id, data: node.data } ) );
	...
	Private.database.node.read = function( req ) {
		...
		Private.log( 'TESTING Private.database.node.read', typeof req.on_success, typeof req.on_error );
	...
	Private.database.index.create = function( req ) {
		...
		var generic_callback = function( err, result ) {
			if( 'undefined' !== typeof err && null !== err ) {
				Private.log('Private.database.index.create generic error',err);
				own_on_error( err );
			} else {
				Private.log('Private.database.index.create generic result',result);
				var generic_callback_2 = function( err2, result2 ) {
					Private.log('Private.database.index.create generic TWO result',result2);

					if( 'undefined' !== typeof err2 && null !== err2 ) {
				
						own_on_error( err2 );
					} else {
						var s = { id: result.start.id, data: result.start.data };
						var e = { id: result.end.id, data: result.end.data };
						console.log("YESSSS",s,e);
						console.log( { id: result.id, data: result.data, start: result.start, end: result.end } );
						own_on_success( { id: result.id, data: result.data, start: result.start, end: result.end } );
						//{ id: result.id, data: result.data, start: { id: result.start.id, data: result.start.data }, end: { id: result.end.id, data: result.end.data }, type: result.type } );
					}
				};
