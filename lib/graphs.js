...
module.exports = ( function() {
	...
	Private.database.relationship = Private.database.relationships || {};
	Private.database.relationship.list = function( req ) {
		Private.log( 'Private.database.list', req );
		var target = 1, current = 0, results = [];

		var generic_callback = function( err, result ) {
			Private.log( 'Private.database.list > generic_callback', err, result );
			if( 'undefined' !== typeof err && null !== err ) {
				own_on_error( err );
				own_on_complete( err );
			} else {
