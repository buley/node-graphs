...
module.exports = ( function() {
	...
	Private.database.query = function( req ) {
		...
		Private.log( 'Private.database.query > checking index', typeof index, index );
		if ( 'undefined' !== typeof index && null !== index ) {
			Private.log( 'Private.database.query > checking index', typeof req.request.index_type, req.request.index_type );
			if ( 'node' === typeof req.request.index_type ) {
				Private.log( 'Private.database.query > doing Private.db.queryNodeIndex()', typeof Private.db.queryNodeIndex, query );
				Private.db.queryNodeIndex( index, query, generic_callback );
			} else if ( 'relationship' === typeof req.request.index_type ) {
				Private.log( 'Private.database.query > doing Private.db.queryRelationshipIndex()', typeof Private.db.queryRelationshipIndex, query );
				Private.db.queryRelationshipIndex( index, query, generic_callback );
			}
		} else {
			Private.log( 'Private.database.query > doing Private.db.query()', typeof Private.db.query, query );
