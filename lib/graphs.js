...
module.exports = ( function() {
	...
	Private.database.query = function( req ) {
		Private.log( 'Private.database.node.query', req );

		var data = req.request.data;
		var index = req.request.index;
		var generic_callback = function( err, result ) {
			Private.log( 'Private.database.node.query > generic_callback', err, result );
		...
		if ( 'undefined' !== typeof index && null !== index ) {
			if ( 'node' === typeof req.request.index_type ) {
				Private.db.queryNodeIndex( data );
			} else if ( 'relationship' === typeof req.request.index_type ) {
				Private.db.queryRelationshipIndex( data );
			}
		} else {
			Private.db.query( data );
		}
