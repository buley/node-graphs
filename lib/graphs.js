...
module.exports = ( function() {
	...
	Private.index.update = function( req ) {
		...
		if ( 'undefined' !== typeof req.data && ( 'undefined' === typeof key || 'undefined' === typeof value || null === key || null === value ) ) {
			target = 0;
			Private.log( 'Private.index.update > looping', req  );
			for( var attr in req.data ) {
				...
				Private.log( 'Private.index.update > looped > doing Private.index.read()', reqobj );
	...
	Private.index.read = function( req ) {
		...
		var own_on_success = function( res ) {
			...
			current += 1;
			if ( 'function' === typeof req.on_success ) {
				Private.log( 'Private.index.read > doing req.on_success()', res );
				req.on_success( req, res, current );
			}
			results.push( res );
			if ( current >= target ) {
		...
		var own_on_complete = function( res, count ) {
			if ( 'function' === typeof req.on_complete ) {
				Private.log( 'Private.index.read > doing req.on_complete()', req );
				req.on_complete( req, results, count );
			}
		};
