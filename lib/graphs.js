			...
			if( 'undefined' !== typeof err && null !== err ) {
				...
				var generic_callback_2 = function( err2, result2 ) {
					...
					if( 'undefined' !== typeof err2 && null !== err2 ) {
						...
						Private.log('Private.database.index.create generic TWO error',err2);
						own_on_error( err2 );
					} else {
						Private.log('Private.database.index.create generic ONE result', { id: result.id, data: result.data, start: { id: result.start.id, data: result.start.data }, end: { id: result.end.id, data: result.end.data }, type: result.type } );
