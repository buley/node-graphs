...
module.exports = ( function() {
	...
	Private.index.update = function( req ) {
		...
		var idx_callback = function( req1, res1 ) {
			console.log( 'Private.index.update > idx_callback() > RESULT', req1, res1  );
		...
		var lookup_callback = function( req0, res0 ) {
			if ( 'relationship' === req.index_type ) {
				console.log( 'Private.index.update > doing Private.database.relationship.update()', req0  );
				Private.database.relationship.update( { request: req0, on_success: idx_callback, on_error: own_on_error } );
			} else {
				console.log( 'Private.index.update > doing Private.database.node.update()', req0  );
		...
		if ( Array.isArray( req.data ) && ( 'undefined' === typeof key || 'undefined' === typeof value || null === key || null === value ) ) {
			...
			target = alen;
			for( a = 0; a < alen; a += 1 ) {
				Private.index.read( { request: req, on_success: lookup_callback, on_error: own_on_error } );
			}
		} else {
			Private.index.read( { request: req, on_success: lookup_callback, on_error: own_on_error } );
		}
