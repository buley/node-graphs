...
module.exports = ( function() {
	...
	Private.database.index.destroy = function( req ) {
		...
		var generic_callback = function( err, result ) {
			if( 'undefined' !== typeof err && null !== err ) {
				...
				Private.log( 'Private.database.index.destroy > doing result2.delete()', typeof result2.delete );
					...
					if ( false !== reindex ) {
						for ( var attr in data ) {
							if ( data.hasOwnProperty( attr ) ) {
								...
								Private.log( 'Private.database.index.update > doing Private.database.index.destroy()' );	
	...
	Private.database.index.read = function( req ) {
		...
		Private.log( 'Private.database.index.read > req.request', req.request );
	...
	Private.index.update = function( req ) {
		...
		var idx_callback = function( req1, res1 ) {
			Private.log( 'Private.index.update > idx_callback() > RESULT', req1, res1  );
		...
		if ( 'undefined' !== typeof req.data && ( 'undefined' === typeof key || 'undefined' === typeof value || null === key || null === value ) ) {
			//TODO: Objects don't have lengths
			target = 0;
			Private.log( 'Private.index.update > doing Private.index.read() > looped', req  );
			for( var attr in req.data ) {
				...
				Private.log( 'Private.index.update > doing Private.index.read() > looped > doing Private.index.read()', reqobj );
				Private.index.read( { request: reqobj, on_success: lookup_callback, on_error: own_on_error } );
			}
		} else {
