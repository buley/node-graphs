	...
	Private.database.index.update = function( req ) {
		...
		var generic_callback = function( err, result ) {
			...
			if( 'undefined' !== typeof err && null !== err ) {
				...
				Private.log( 'Private.database.index.update err', err );
				own_on_error( err );
				...
				var very_own_on_success = function( res2 ) {
					...
					if ( true === reindex ) {
						...
						if ( 'undefined' !== typeof res2.reference.index ) {
							...
							console.log("NOT REINDEXING OWN");
		...
		if ( 'relationship' === type ) {
			console.log("GETTTTING REALLLLLLLLLLLLLLLLL", type, indes, key, value );
