...
module.exports = ( function() {
	...
	Private.database.relationship = Private.database.relationship || {};

	Private.database.relationship.list = function( req ) {
		Private.log( 'Private.database.relationship.list', req );
		var target = 1, current = 0, results = [];

		var generic_callback = function( err, result ) {
			Private.log( 'Private.database.relationship.list > generic_callback', err, result );
			if( 'undefined' !== typeof err && null !== err ) {
				own_on_error( err );
				own_on_complete( err );
			} else {
		...
		Private.db.listRelationshipTypes( generic_callback );
	...
	Private.database.node.list = function( req ) {
		...
		if ( 'all' === direction || null === direction || 'undefined' === typeof direction  ) {

			if( 'undefined' !== typeof types && 'string' !== typeof types && types.length > 0 ) {
			...
			if( 'undefined' !== typeof types && 'string' !== typeof types && types.length > 0 ) {
				...
				Private.db.listNodeRelationships( generic_callback, id, 'out', types );
			} else {
				//any outgoing relationships
				//http://localhost:7474/db/data/node/1/relationships/out
				Private.db.listNodeRelationships( generic_callback, id, 'out' );
			}
			...
			if( 'undefined' !== typeof types && 'string' !== typeof types && types.length > 0 ) {
				...
				Private.db.listNodeRelationships( generic_callback, id, 'in', types );
			} else {
				//incoming relationships
				//http://localhost:7474/db/data/node/2/relationships/in/
				Private.db.listNodeRelationships( generic_callback, id, 'in' );
			}
	...
	Private.database.node.create = function( req ) {
		...
		var generic_callback = function( err, node ) {
			Private.log( 'Private.database.node.create > generic_callback', err, node);
			if( 'undefined' !== typeof err && null !== err ) {
				own_on_error( err );
			} else {
				if ( null !== node ) {
					...
					own_on_success( { id: null, data: null, reference: null } );
				}
			}
		};
		var own_on_success = function( res ) {
	...
	Private.node.list = function( req ) {

		Private.log( 'Private.node.list', req );
		...
		if ( Array.isArray( req.data ) && ( 'undefined' === typeof key || 'undefined' === typeof value || null === key || null === value ) ) {
			...
			for( a = 0; a < alen; a += 1 ) {
				Private.database.node.list( { request: req, on_success: own_on_success, on_error: own_on_error } );
			}
	...
	Private.relationship.list = function( req ) {

		Private.log( 'Private.relationship.list', req );
		...
		if ( Array.isArray( req.data ) && ( 'undefined' === typeof key || 'undefined' === typeof value || null === key || null === value ) ) {
			...
			for( a = 0; a < alen; a += 1 ) {
				Private.database.relationship.list( { request: req, on_success: own_on_success, on_error: own_on_error } );
			}
	...
	Private.index.read = function( req ) {
		...
		var own_on_success = function( res ) {
			...
			if ( 'function' === typeof req.on_success ) {
				Private.log( 'Private.index.read > doing req.on_success()', req );
				req.on_success( req, res );
			}
		};
		...
		var own_on_complete = function( res ) {
			if ( 'function' === typeof req.on_complete ) {
				Private.log( 'Private.index.read > doing req.on_complete()', req, results );
				req.on_complete( req, res );
			}
		};
		...
		if ( ( 'undefined' !== typeof req.data && null !== req.data ) && ( 'undefined' === typeof key || 'undefined' === typeof value || null === key || null === value ) ) {
			var a = 0, alen = 0, aitem;
			for ( aitem in req.data ) {
				if ( req.data.hasOwnProperty( aitem ) ) {
					alen += 1;	
				}
			}
			target = alen;
			for( aitem in req.data ) {
				if ( req.data.hasOwnProperty( aitem ) ) {
					...
					reqobj.key = aitem;
					reqobj.value = req.data[ aitem ];
					reqobj.index = index;
					reqobj.index_type = index_type;
					Private.log( 'Private.index.read > doing Private.database.index.read()', req );
					Private.database.index.read( { request: reqobj, on_success: own_on_success, on_error: own_on_error, on_complete: own_on_complete } );
				}
			}
		} else {
			var reqobj = {};
	...
	Public.prototype.put = function( req ) {
		Private.log( 'Public.prototype.put', req );
		if ( true !== Private.connected ) this.connect();
		var datatype = req.datatype;
		delete req.datatype;

		req.find.on_success = (function() {
			fn = req.on_success;
			return function(oreq, ores) {
				console.log('EXISTING',ores.id);
				var old = ores.data;
				var attr;
				req.update.data = req.update.data || {};
				for ( attr in old ) {
					if ( 'undefined' === typeof req.update.data[ attr ] ) {
						req.update.data[ attr ] = old[ attr ];
					}
				}
				req.update.on_success = (function() {
					return function() {
				...
				if ( 'node' === datatype ) {
					Private.node.update( req.update );
					//console.log("UPDATE EXISTING NODE");
				} else if ( 'relationship' === datatype ) {
					Private.relationship.update( req.update );
				} else if ( 'index' === datatype ) {
					Private.index.update( req.update );
				}

			}
		})();

		req.find.on_error = (function() {
			fn = req.on_success;
			return function(oreq, ores) {
				...
				if ( 'node' === datatype ) {
					Private.node.create( req.update );
					//console.log("CREATE NEW NODE");
				} else if ( 'relationship' === datatype ) {
					Private.relationship.create( req.update );
				} else if ( 'index' === datatype ) {
					Private.index.create( req.update );
				}
		...
		if ( 'node' === datatype ) {
			Private.node.read( req.find );
		} else if ( 'relationship' === datatype ) {
			Private.relationship.read( req.find );
		} else if ( 'index' === datatype ) {
			Private.index.read( req.find );
		}
	};
