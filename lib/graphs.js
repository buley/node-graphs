	...
	Private.database.node.create = function( req ) {
		...
		var generic_callback = function( err, result ) {
			...
			if( 'undefined' !== typeof err && null !== err ) {
				...
				Private.log( 'generic callback success > ' + JSON.stringify( { id: node.id, data: node.data, node: node } ) );
				own_on_success( { id: node.id, data: node.data, node: node } );
			}
		};
			...
			if ( 'undefined' !== typeof err && null !== err ) {
				...
				var own_generic_callback = function( err2, result2 ) {
					if ( 'undefined' !== typeof err2 && null !== err2 ) {
						...
						own_on_success( { id: result.id, data: result.data, node: result } );
					}
				};
				result.data = data;
	...
	Private.database.relationship.update = function( req ) {
		...
		var generic_callback = function( err, result ) {
			...
			if( 'undefined' !== typeof err && null !== err ) {
				...
				Private.log( 'generic callback success > ' + JSON.stringify( { id: result.id, data: result.data, node: result } ) );

				var own_generic_callback = function( err2, result2 ) {
					if( 'undefined' !== typeof err2 && null !== err2 ) {
						own_on_error( err2 );
					} else {
						own_on_success( { id: result.id, data: result.data, start: { id: result.start.id, data: result.start.data }, end: { id: result.end.id, data: result.end.data }, type: result.type, node: result } );
					}
				};
				result.data = data;
				result.save( own_generic_callback );
			...
			if( 'undefined' !== typeof err && null !== err ) {
				...
				var very_own_on_success = function( res2 ) {
					...
					var very_very_own_on_success = function( res3 ) {
						count = count + 1;
						Private.log( 'Private.database.index.update > very_very_own_on_success', res2 );
						if ( x === target ) {
							own_on_success( res3 );
						}	
					};
					var very_very_own_on_error = function( res3 ) {
						Private.log( 'Private.database.index.update > very_very_own_on_error', res2 );
						if ( x === target ) {
							own_on_error( res3 );
						}	
					};
					if ( true === reindex ) {
						Private.log( "DOING REINDEX", id, index, data );
						...
						for ( var attr in data ) {
							if ( data.hasOwnProperty( attr ) ) {
								res2.index( index, attr, data[ attr ], generic_callback_2 );
							}
						}
				...
				Private.log('Private.database.index.update',result);
			...
			if( 'undefined' !== typeof err && null !== err ) {
				...
				if ( count > 0 ) {
					for ( var x = 0; x < count; x += 1 ) {
						if ( type === 'relationship' ) {
							own_on_success( { id: result[ x ].id, data: result[ x ].data, start: result[ x ].start, end: result[ x ].end, type: result[ x ].type, node: result[ x ] } );
							items.push( { id: result[ x ].id, data: result[ x ].data, start: result[ x ].start, end: result[ x ].end, type: result[ x ].type, node: result[ x ] } );
						} else {
							own_on_success( { id: result[ x ].id, data: result[ x ].data, node: result[ x ] } );
							items.push( { id: result[ x ].id, data: result[ x ].data, node: result[ x ] } );
						}
					}
				} else {
					own_on_success( null );
