...
module.exports = ( function() {
	...
	Private.database.index.destroy = function( req ) {
		...
		Private.log( 'Private.database.index.destroy > doing Private.db.deleteRelationshipEntry', typeof Private.db.deleteRelationshipEntry, index, key, value );
					...
					if ( false !== reindex ) {
						for ( var attr in data ) {
							if ( data.hasOwnProperty( attr ) ) {
								...
								Private.log( 'Private.database.index.update > doing Private.database.index.destroy() > PREVIOUS', ( 'undefined' !== typeof previous ? previous : null ) );	
								vvv_generic_callback();
								for( var attr3 in previous ) {
									target = target + 1;
									Private.log( 'Private.database.index.update > doing Private.database.index.destroy() > PREVIOUS > ITEM', attr3 );	
	...
	Private.database.index.read = function( req ) {
		...
		var generic_callback = function( err, result ) {
			...
			var previous = JSON.parse( JSON.stringify( res2.reference.data ) );
			Private.log( 'Private.database.index.update > generic_callback() > very_own_on_success() > PREVIOUS', previous );
	...
	Private.index.update = function( req ) {
		...
		var idx_callback = function( req1, res1 ) {
			...
			if ( true === req.reindex ) {
				Private.log( 'Private.index.update > idx_callback() > doing Private.database.index.update()', reqobj );
